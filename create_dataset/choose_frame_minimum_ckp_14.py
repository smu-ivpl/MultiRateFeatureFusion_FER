...
def create_moving_batch(images_list, slow_frame, fast_frame, middle_frame):
    ...
    sinterval = (len(images_list) - 1 - 2) / (slow_frame - 1)
    finterval = (len(images_list) - 1 - 2) / (fast_frame - 1)
    minterval = (len(images_list) - 1 - 2) / (middle_frame - 1)
    ...
    for index, image in enumerate(sorted(images_list, key=last_8chars)):
        ...
        for j in range(fast_frame):
            ...
            if index == 1:
                batch2[0][0] = image
                j += 1
            elif index == 1 + round(finterval * j):
                batch2[0][j] = image
                j += 1

        for l in range(middle_frame):
            # neutral images
            if (index == 0):
                batch3[1][0] = image
            elif (index == 1):
            ...
            if index == 2:
                batch3[0][0] = image
                l += 1
            elif index == 2 + round(minterval * l):
                batch3[0][l] = image
                l += 1
...
def main(argv):
    gen_emotionsFolders(
        image_main_dir=str(argv[0]),  # ckp/cohn-kanade-images/ : 蹂??��?�???�쏙??吏 ??�쎈??        
        emotions_dir=str(argv[1]),  # ckp/emotion_images/ : ??�쎈�?留뚮�??�쎌�???�쎈??        
